...
import { Award, TrendingUp, CheckCircle2, AlertCircle, Sparkles, User, FileText, Loader2 } from 'lucide-react';
// Pastikan jalur impor ini sesuai letak file-mu, misalnya '../supabaseClient'
...
const EvaluationModule: React.FC<EvaluationModuleProps> = ({ role, userData }) => {
  ...
  // Mengambil daftar siswa dari database untuk dropdown Guru
  useEffect(() => {
    if (role === 'guru') {
      const fetchStudents = async () => {
        ...
        try {
          ...
          if (data && data.length > 0) {
            ...
            setSelectedStudent(data[0].id); // Set default pilihan ke siswa pertama
          }
        } catch (error) {
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-rose-200 text-sm">
              {role === 'siswa' 
                ? `Rekap jejak belajar untuk ${userData?.nama_lengkap || userData?.name || 'Siswa'}` 
                : 'Pantauan rekapitulasi nilai dan kemajuan seluruh siswa'}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-6 rounded-3xl border border-rose-100 shadow-sm flex items-center gap-4">
              <div className="w-14 h-14 bg-rose-50 text-rose-800 rounded-2xl flex items-center justify-center font-bold text-xl">84</div>
              <div>
            ...
            {/* Kartu 2: Perkembangan */}
            <div className="bg-white p-6 rounded-3xl border border-rose-100 shadow-sm flex items-center gap-4">
              <div className="w-14 h-14 bg-rose-50 text-rose-800 rounded-2xl flex items-center justify-center"><TrendingUp size={24} /></div>
              <div>
            ...
            {/* Kartu 3: Status */}
            <div className="bg-white p-6 rounded-3xl border border-rose-100 shadow-sm flex items-center gap-4">
              <div className="w-14 h-14 bg-rose-50 text-rose-800 rounded-2xl flex items-center justify-center"><CheckCircle2 size={24} className="text-rose-800" /></div>
              <div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-rose-100 overflow-hidden">
            <div className="p-5 border-b border-rose-50 bg-rose-50/30 flex items-center justify-between">
              <h3 className="font-bold text-rose-950 flex items-center gap-2"><FileText size={18} className="text-rose-800" /> Rincian Nilai</h3>
            </div>
            <div className="divide-y divide-rose-50">
              {studentGrades.map((item, index) => (
                <div key={index} className="p-4 md:p-5 flex items-center justify-between hover:bg-rose-50/20 transition-colors">
                  ...
                  <span className="px-3 py-1 bg-rose-100 text-rose-900 font-bold text-sm rounded-full">{item.score} Poin</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl shadow-sm border border-rose-100 p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-rose-50">
              <div>
                <h3 className="font-bold text-rose-950 text-lg">Rekapitulasi Nilai (Database)</h3>
                <p className="text-slate-500 text-sm">Menampilkan data siswa asli dari Supabase.</p>
              </div>
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan={5} className="p-6 text-center text-slate-400">
                        <Loader2 className="animate-spin mx-auto mb-2 text-rose-500" size={24} /> Memuat data...
                      </td>
                    </tr>
                  ) : studentsList.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-6 text-center text-slate-400">Belum ada siswa terdaftar.</td>
                    </tr>
                  ) : (
                    studentsList.map((row) => (
                      <tr key={row.id} className="hover:bg-rose-50/20 transition-colors">
                        <td className="p-3 font-semibold text-slate-800 flex items-center gap-2">
                          <User size={16} className="text-rose-800" /> {row.nama_lengkap}
                        </td>
                        <td className="p-3 font-medium text-slate-600">85</td>
                        <td className="p-3 font-medium text-slate-600">90</td>
                        <td className="p-3 font-medium text-slate-600">78</td>
                        <td className="p-3">
                          <span className="px-3 py-1 text-xs font-bold rounded-full bg-emerald-100 text-emerald-800">Sangat Baik</span>
                        </td>
                      </tr>
                    ))
                  )}
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-rose-100 p-6">
            <h4 className="font-bold text-rose-950 mb-3 flex items-center gap-2">
              <AlertCircle size={18} className="text-rose-800" /> Rencana Tindak Lanjut Pembelajaran
            </h4>
            <p className="text-sm text-slate-600 leading-relaxed mb-4">
              Sebagian besar siswa telah menguasai materi dasar. Namun, terdapat beberapa siswa yang memerlukan sesi pengayaan (*remedial*) terutama pada pemecahan soal cerita SPLDV.
            </p>
